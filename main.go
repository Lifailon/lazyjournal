...
func (app *App) loadDockerLogs(containerName string, newUpdate bool) {
	...
	if !readFileContainer || containerizationSystem == "podman" || containerizationSystem == "kubectl" {
		...
		if containerizationSystem == "kubectl" {
			...
			switch {
			case app.sinceTimestampFilterMode && app.untilTimestampFilterMode:
				cmd = exec.Command(containerizationSystem, "logs", "--timestamps", "--since", sinceFilterTextNotSpace, "--until", untilFilterTextNotSpace, "--tail", app.logViewCount, containerId)
			case app.sinceTimestampFilterMode && !app.untilTimestampFilterMode:
				cmd = exec.Command(containerizationSystem, "logs", "--timestamps", "--since", sinceFilterTextNotSpace, "--tail", app.logViewCount, containerId)
			case !app.sinceTimestampFilterMode && app.untilTimestampFilterMode:
				cmd = exec.Command(containerizationSystem, "logs", "--timestamps", "--until", untilFilterTextNotSpace, "--tail", app.logViewCount, containerId)
			default:
...
func (app *App) setupKeybindings() error {
	...
	// Включение/выключение режима покраски через tailspin (Ctrl+N)
	if err := app.gui.SetKeybinding("", gocui.KeyCtrlN, gocui.ModNone, func(g *gocui.Gui, v *gocui.View) error {
		if app.tailSpinMode {
	...
	// F1
	if err := app.gui.SetKeybinding("", gocui.KeyF1, gocui.ModNone, helpHandler); err != nil {
		return err
	}
	// ?
	if err := app.gui.SetKeybinding("", '?', gocui.ModNone, helpHandler); err != nil {
		return err
	}
	...
	// Переключение режима вывода потоков журналов Docker (Ctrl+S)
	...
	// Переключение режима чтения журналов Docker из потоков или файловой системы (Ctrl+D)
...
func (app *App) showInterfaceHelp(g *gocui.Gui) {
	...
	fmt.Fprintln(helpView, "    \033[32m<Shift/Alt/Ctrl>+<k/j>\033[0m - quickly move up and down (like Vim and alternative for macOS).")
